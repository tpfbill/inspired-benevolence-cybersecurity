...
const verifyNetworkIoTPlaybook = async () => {
  try {
    ...
    const playbook = await Playbook.findOne({
      where: {
        name: 'Network IoT Incident Response'
      }
    });

    if (!playbook) {
      console.log('❌ Network IoT Incident Response playbook not found!');
      process.exit(1);
    }

    console.log('\n✓ Playbook found: Network IoT Incident Response');
