...
const verifyRansomwarePlaybook = async () => {
  try {
    await sequelize.authenticate();
    console.log('Database connected');

    // Find the Ransomware Response playbook
    ...
    if (!playbook) {
      console.log('❌ Ransomware Response playbook not found!');
      process.exit(1);
    }

    console.log('\n✓ Playbook found: Ransomware Response');
